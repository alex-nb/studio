/*  Authorization  */

export const authTypes = {
    USER_LOAD : 'USER_LOAD',
    REGISTER_SUCCESS : 'REGISTER_SUCCESS',
    LOGIN_SUCCESS : 'LOGIN_SUCCESS',
    REGISTER_FAIL : 'REGISTER_FAIL',
    AUTH_ERROR : 'AUTH_ERROR',
    LOGIN_FAIL : 'LOGIN_FAIL',
    LOGOUT : 'LOGOUT'
};


/*   Common info */

export const commonInfoTypes = {
    CHANGE_SELECTED_MENU: 'CHANGE_SELECTED_MENU'
};


/*   Employees page */

export const employeesPageTypes = {
    EMPLOYEES_GET_SUCCESS : 'EMPLOYEES_GET_SUCCESS',
    EMPLOYEES_GET_REQUEST : 'EMPLOYEES_GET_REQUEST',
    EMPLOYEES_GET_FAILURE : 'EMPLOYEES_GET_FAILURE',
    DEPARTMENTS_GET_SUCCESS : 'DEPARTMENTS_GET_SUCCESS',
    DEPARTMENTS_GET_REQUEST : 'DEPARTMENTS_GET_REQUEST',
    DEPARTMENTS_GET_FAILURE : 'EMPLOYEES_GET_SUCCESS',
    DEPARTMENT_ORDER_GET_SUCCESS : 'DEPARTMENT_ORDER_GET_SUCCESS',
    DEPARTMENT_ORDER_GET_REQUEST : 'DEPARTMENT_ORDER_GET_REQUEST',
    DEPARTMENT_ORDER_GET_FAILURE : 'DEPARTMENT_ORDER_GET_FAILURE',
    ALL_EMPLOYEES_LIST_GET_SUCCESS : 'ALL_EMPLOYEES_LIST_GET_SUCCESS',
    ALL_EMPLOYEES_LIST_GET_REQUEST : 'ALL_EMPLOYEES_LIST_GET_REQUEST',
    ALL_EMPLOYEES_LIST_GET_FAILURE : 'ALL_EMPLOYEES_LIST_GET_FAILURE'
};


/*   Expenditures page */


export const expendituresPageTypes = {
    EXPENDITURE_GET_REQUEST : 'EXPENDITURE_GET_REQUEST',
    EXPENDITURE_GET_SUCCESS : 'EXPENDITURE_GET_SUCCESS',
    EXPENDITURE_GET_FAILURE : 'EXPENDITURE_GET_FAILURE'
};

/*   Projects page */

export const projectsPageTypes = {
    PROJECTS_NEW_GET_SUCCESS : 'PROJECTS_NEW_GET_SUCCESS',
    PROJECTS_NEW_GET_REQUEST : 'PROJECTS_NEW_GET_REQUEST',
    PROJECTS_NEW_GET_FAILURE : 'PROJECTS_NEW_GET_FAILURE',
    PROJECTS_PROCESS_GET_SUCCESS : 'PROJECTS_PROCESS_GET_SUCCESS',
    PROJECTS_PROCESS_GET_REQUEST : 'PROJECTS_PROCESS_GET_REQUEST',
    PROJECTS_PROCESS_GET_FAILURE : 'PROJECTS_PROCESS_GET_FAILURE',
    PROJECTS_CLOSE_GET_SUCCESS : 'PROJECTS_CLOSE_GET_SUCCESS',
    PROJECTS_CLOSE_GET_REQUEST : 'PROJECTS_CLOSE_GET_REQUEST',
    PROJECTS_CLOSE_GET_FAILURE : 'PROJECTS_CLOSE_GET_FAILURE',
    PERSONAL_REPORTS_GET_SUCCESS : 'PERSONAL_REPORTS_GET_SUCCESS',
    PERSONAL_REPORTS_GET_REQUEST : 'PERSONAL_REPORTS_GET_REQUEST',
    PERSONAL_REPORTS_GET_FAILURE : 'PERSONAL_REPORTS_GET_FAILURE'
};



/*   Reports page */

export const reportsPageTypes = {
    ALL_REPORTS_GET_REQUEST : 'ALL_REPORTS_GET_REQUEST',
    ALL_REPORTS_GET_SUCCESS : 'ALL_REPORTS_GET_SUCCESS',
    ALL_REPORTS_GET_FAILURE : 'ALL_REPORTS_GET_FAILURE'
};


/*   Requests money page */

export const requestsMoneyPageTypes = {
    REQUESTS_MONEY_GET_REQUEST : 'REQUESTS_MONEY_GET_REQUEST',
    REQUESTS_MONEY_GET_SUCCESS : 'REQUESTS_MONEY_GET_SUCCESS',
    REQUESTS_MONEY_GET_FAILURE : 'REQUESTS_MONEY_GET_FAILURE'
};


/*   Transactions page */

export const transactionsPageTypes = {
    TRANSACTION_GET_REQUEST : 'TRANSACTION_GET_REQUEST',
    TRANSACTION_GET_SUCCESS : 'TRANSACTION_GET_SUCCESS',
    TRANSACTION_GET_FAILURE : 'TRANSACTION_GET_FAILURE'
};


/*   Header */

export const headerPageTypes = {
    PERSONAL_INFO_GET_REQUEST : 'PERSONAL_INFO_GET_REQUEST',
    PERSONAL_INFO_GET_SUCCESS : 'PERSONAL_INFO_GET_SUCCESS',
    PERSONAL_INFO_GET_FAILURE : 'PERSONAL_INFO_GET_FAILURE'
};